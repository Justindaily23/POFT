...
export class PoImportController {
  ...
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles(AuthRole.ADMIN, AuthRole.SUPER_ADMIN)
  async getHistory(@Query('limit') limit?: string, @Query('status') status?: string): Promise<PoImportHistory[]> {
    return await this.importService.getImportHistory(limit, status);
  }
