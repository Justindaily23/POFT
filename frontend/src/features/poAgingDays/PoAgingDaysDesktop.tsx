...
import { Search, ChevronDown, ChevronUp, Filter, Calendar, ChevronLeft, ChevronRight, X } from "lucide-react";
...
const generateMockData = (count: number, pmId?: string): PoAgingDaysResponse[] => {
    ...
    const projectNames = ["Enterprise Platform", "Mobile App Redesign", "Data Migration", "Cloud Infrastructure", "Security Upgrade"];
...
const PoAgingDaysDesktop: React.FC<Props> = ({ userRole, currentPmId }) => {
        ...
        if (userRole === "ADMIN") {
            if (searchPM.trim()) {
                filtered = filtered.filter(
                    (po) => po.pm.toLowerCase().includes(searchPM.toLowerCase()) || po.pmId.toLowerCase().includes(searchPM.toLowerCase()),
                );
            }
            if (searchDUID.trim()) {
                filtered = filtered.filter((po) => po.duid.toLowerCase().includes(searchDUID.toLowerCase()));
            }
            if (searchPONumber.trim()) {
                filtered = filtered.filter((po) => po.poNumber.toLowerCase().includes(searchPONumber.toLowerCase()));
            }
            if (searchProjectCode.trim()) {
                filtered = filtered.filter((po) => po.projectCode.toLowerCase().includes(searchProjectCode.toLowerCase()));
            }
            if (searchProjectName.trim()) {
                filtered = filtered.filter((po) => po.projectName.toLowerCase().includes(searchProjectName.toLowerCase()));
            }
        }
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [searchPM, searchDUID, searchPONumber, searchProjectCode, searchProjectName, filterAgingFlag, filterInvoiceStatus, filterPOType]);
            ...
            <div className="max-w-480 mx-auto p-8">
                ...
                <div className="mb-10">
                    <div className="flex items-center justify-between mb-2">
                        <h1 className="text-4xl font-bold text-slate-900 tracking-tight">Purchase Order Aging Analysis</h1>
                        <div className="text-sm text-slate-500 font-medium">
                            {userRole === "ADMIN" ? "Administrator View" : "Project Manager View"}
                        </div>
                    </div>
                    <p className="text-slate-600 text-base">Monitor and analyze purchase order aging across all projects</p>
                </div>
                    ...
                    {userRole === "ADMIN" && (
                        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                            <div className="flex items-center justify-between mb-3">
                                <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Total POs</div>
                                <div className="w-10 h-10 bg-indigo-100 rounded-lg flex items-center justify-center">
                                    <div className="w-2 h-2 bg-indigo-600 rounded-full"></div>
                                </div>
                            </div>
                            <div className="text-3xl font-bold text-slate-900 mb-1">{metrics.totalPOs.toLocaleString()}</div>
                            <div className="text-xs text-slate-500">Purchase orders</div>
                        </div>
                    )}
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between mb-3">
                            <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Invoiced</div>
                            <div className="w-10 h-10 bg-emerald-100 rounded-lg flex items-center justify-center">
                                <div className="w-2 h-2 bg-emerald-600 rounded-full"></div>
                            </div>
                        </div>
                        <div className="text-3xl font-bold text-emerald-600 mb-1">{metrics.invoicedPOs.toLocaleString()}</div>
                        <div className="text-xs text-slate-500">Completed invoices</div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between mb-3">
                            <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Not Invoiced</div>
                            <div className="w-10 h-10 bg-orange-100 rounded-lg flex items-center justify-center">
                                <div className="w-2 h-2 bg-orange-600 rounded-full"></div>
                            </div>
                        </div>
                        <div className="text-3xl font-bold text-orange-600 mb-1">{metrics.notInvoicedPOs.toLocaleString()}</div>
                        <div className="text-xs text-slate-500">Pending invoices</div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between mb-3">
                            <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Invoice Rate</div>
                            <div className="w-10 h-10 bg-blue-100 rounded-lg flex items-center justify-center">
                                <div className="w-2 h-2 bg-blue-600 rounded-full"></div>
                            </div>
                        </div>
                        <div className="text-3xl font-bold text-blue-600 mb-1">{metrics.invoiceRate.toFixed(1)}%</div>
                        <div className="text-xs text-slate-500">Completion rate</div>
                    </div>
                    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 hover:shadow-md transition-shadow">
                        <div className="flex items-center justify-between mb-3">
                            <div className="text-slate-500 text-xs font-bold uppercase tracking-wider">Avg Aging</div>
                            <div className="w-10 h-10 bg-purple-100 rounded-lg flex items-center justify-center">
                                <div className="w-2 h-2 bg-purple-600 rounded-full"></div>
                            </div>
                        </div>
                        <div className="text-3xl font-bold text-purple-600 mb-1">{metrics.avgPoAgingDays.toFixed(1)}</div>
                    ...
                    <div className="p-6">
                        ...
                        {userRole === "ADMIN" && (
                            <div className="grid grid-cols-5 gap-4 mb-6">
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Search PM</label>
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Search DUID</label>
                                    <div className="relative">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                        <input
                                            type="text"
                                ...
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Search PO Number</label>
                                    <div className="relative">
                                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                                        <input
                                            type="text"
                        ...
                        <div className="grid grid-cols-3 gap-4">
                            {userRole === "ADMIN" && (
                                <div>
                                    <label className="block text-sm font-semibold text-slate-700 mb-2">Aging Flag</label>
                                    <select
                ...
                <div className="mb-4 flex items-center justify-between">
                    <div className="text-sm text-slate-600">
                        ...
                        <span className="font-bold text-slate-900">{filteredAndSearchedData.length}</span> results
                                ...
                                {isLoadingPO ? (
                                    ...
                                    paginatedData.map((po, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-4 py-3 text-sm text-slate-900 font-semibold">{po.poNumber}</td>
                                            ...
                                            <td className="px-4 py-3 text-sm text-slate-700 text-center">{po.poLineNumber}</td>
                                            <td className="px-4 py-3 text-sm">
                                                <span className="px-2 py-1 bg-slate-100 text-slate-700 rounded-md text-xs font-medium">
                                                    {po.poType}
                                                </span>
                                            </td>
                                            <td className="px-4 py-3 text-sm text-slate-700">{formatDate(po.poIssuedDate)}</td>
                                            <td className="px-4 py-3 text-sm text-slate-700">{formatDate(po.poInvoiceDate)}</td>
                                            <td className="px-4 py-3 text-sm text-slate-700 text-center">{po.allowedOpenDays}</td>
                                            ...
                                            <td className="px-4 py-3 text-sm text-slate-600 max-w-xs truncate" title={po.itemDescription}>
                                                {po.itemDescription}
                                            </td>
                                            <td className="px-4 py-3 text-sm">
                                                <span
