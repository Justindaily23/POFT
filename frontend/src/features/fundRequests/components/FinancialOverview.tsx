...
export default function FinancialOverview({ selectedPOLine, requestedAmount }: Props) {
  const [showDetails, setShowDetails] = useState(false); // NEW: mobile toggle
  ...
  // Extra details you want hidden on mobile
  const extraDetails = {
        ...
        <>
          ...
          <div className="bg-linear-to-br from-blue-50 to-blue-100/50 rounded-xl p-4 border border-blue-200">
            <div className="flex items-center gap-2 mb-1">
              ...
              {/* Toggle for extra details on mobile */}
              <button className="sm:hidden" onClick={() => setShowDetails(!showDetails)}>
                {showDetails ? (
            ...
            {showDetails && (
              <div className="mt-2 text-[10px] text-blue-700 space-y-1 sm:hidden">
